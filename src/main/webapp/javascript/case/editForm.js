	...
	var locale = $("#app-locale").text();
	 ...
	 $('#viewComplaint').ajaxForm({
		...
		function submit(btn){
			...
			if($('#viewComplaint').valid()){
			      ...
			      $.ajax({
						url:contextPath+"case/update",
						method : "POST",
						data :[{"name": "id", "value" : $("#id").val() },{"name": "statusId", "value" : $("#status").val() },{"name": "priorityId", "value" : $("#priority").val() },{"name": "message", "value" : $("#note").val() }],
	           ...
	           "aoColumnDefs": [
	                           
	                            {
	                                ...
	                                "fnRender" : function ( oObj ) {
	                                	...
	                                	res='<strong>'+oObj.aData.status+'</strong><p class="text-success">Last Status Update : <strong>'+oObj.aData.lastUpdate+'</strong></p>';
	                                	if(oObj.aData.status=="SOLVED"){
	                                		res='<strong>'+oObj.aData.status+'</strong><p class="text-success">Last Status Update : <strong>'+oObj.aData.lastUpdate+'</strong></p><p><button type="button" href="'+contextPath+'case/assignment/reassign/'+oObj.aData.assignmentId+'" class="reassign btn btn-success">RE-ASSIGN ?</button></p>';
	                                	}
	                                	
	                                	
	                                	if(oObj.aData.statusM=="RE-OPEN"){
	                                		$("#status").val(oObj.aData.statusM).change();
	                                		$("#komen").show();
	                                	}else{
	                                		$("#komen").hide();
	                                	}
	                            ...
	                            },
	                            {
	                                "bUseRendered": false,
	                                "fnRender" : function ( oObj ) {
	                               	return '<a class="logClick" href="'+contextPath+'eventLog/assignment/'+oObj.aData.assignmentId+'/'+oObj.aData.assignmentId+'"><i class="i-cabinet"></i></a>';
	                                },
		...
		$('#assignBtn').click(function(e){
			e.preventDefault();
			var btn=$(this);
			confirmNoty(btn,"Anda pasti untuk tugaskan kepada jabatan ?",assign);
...
function confirmIt(btn,form,text){
