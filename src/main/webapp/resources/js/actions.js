$(document).ready(function(){
    ...
    /* EOF Settings */
        ...
        var sOpen = $.cookies.get('sOpen');
        if(sOpen!=null){
        	var ob=$('.openable');
        	var id=ob.attr('menuId');
       ...
       // 	alert($(this).attr('menuId'));
        	// $.cookies.set('sOpen',$(this).attr('menuId'));
        });
...
function block_loading(content){
    ...
    if(content.find('.block_loading').length > 0)
        ...
        content.append('<div class="block_loading" style="width: '+(content.width()+ptc)+'px; height: '+(content.height()+ptc)+'px;"><img src="'+contextPath+'resources/img/loader.gif"/></div>');
    }
    return false;
}
...
function confirm(btn,form,text){
...
function confirmNoty(btn,text,callBack){
	 $(btn).attr('disabled',true);
		noty({
	        text: text,
	        type: 'information',
